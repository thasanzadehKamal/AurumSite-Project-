...
datas.slice(12,24).forEach(item => {
  cards.innerHTML += `
    <div class="card">
      <div id="img">
        <img
          src="${item.img}"
          onerror="this.src='https://cdn.dribbble.com/userupload/28628569/file/original-edbc9b1a905204e54ac50ca36215712a.jpg'" alt="">
      </div>
      <div id="header">
        <h1 class="text-[35px]">${item.name}</h1>
        <p>
         ${item.dsc}
        </p>
      </div>
      <div id="foot">
        <div>
          <p class="text-[20px]">Price: ${item.price}</p>
        </div>
...
// ------------------------------------------------
let srch = document.querySelector(".nav-search")
...
// -------------------------------------------------
let close = document.getElementById("basketclose")
...
// })
// -------------------------------------------------
let addproduct = document.getElementsByClassName("add-button")
addproduct.addEventListener("click", () => {
  quickbasket.innerHTML += `
     <li
          class="min-w-0 flex items-start gap-2.5 sm:gap-3 rounded-2xl border border-[#c9a84c2e] bg-white/5 p-[10px] sm:p-3">
          <img
            class="quickbasket-image h-[36px] w-[36px] shrink-0 sm:h-[44px] sm:w-[44px] rounded-lg sm:rounded-xl object-cover"
            src="${item.img}"
            alt="Product image">
          <div class="min-w-0 flex-1">
            <h4 class="m-0 mb-1 text-[15px] sm:text-[17px] text-[#f6e9cf] not-italic leading-snug break-words">${item.name}</h4>
            <p class="m-0 text-[14px] sm:text-[16px] text-[#f8d98b] not-italic">${item.price}</p>
            <div class="mt-2 flex items-center gap-1.5 sm:gap-2">
              <button
                class="h-6 w-6 sm:h-7 sm:w-7 rounded-lg border border-[#c9a84c73] bg-white/[0.03] text-[16px] sm:text-[18px] leading-none text-[#f7efe2] cursor-pointer">&minus;</button>
              <button
                class="h-6 min-w-6 px-1 sm:h-7 sm:min-w-7 rounded-lg border border-[#c9a84c73] bg-white/[0.03] text-[14px] sm:text-[16px] leading-none text-[#f7efe2]">1</button>
              <button
                class="h-6 w-6 sm:h-7 sm:w-7 rounded-lg border border-[#c9a84c73] bg-white/[0.03] text-[16px] sm:text-[18px] leading-none text-[#f7efe2] cursor-pointer">&plus;</button>
            </div>
          </div>
        </li>`
})
